...
<div class="pageHeader">
    <form id="pagerForm" onsubmit="return navTabSearch(this);" action="<%=basePath%>admin/consume_toConsumeIndex.do"
          method="post">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <select class="combox" name="tradeType" style="float: none;">
                            ...
                            <option value="2" <c:if test="${tradeType eq 2}">selected="selected"</c:if>>团购</option>
                            <option value="5" <c:if test="${tradeType eq 5}">selected="selected"</c:if>>POS机刷卡</option>
                            <option value="6" <c:if test="${tradeType eq 6}">selected="selected"</c:if>>缴纳物业费</option>
                            <option value="7" <c:if test="${tradeType eq 7}">selected="selected"</c:if>>缴纳停车费</option>
                            <option value="3" <c:if test="${tradeType eq 3}">selected="selected"</c:if>>转账-转出</option>
                            <option value="4" <c:if test="${tradeType eq 4}">selected="selected"</c:if>>转账-转入</option>
                            <option value="9" <c:if test="${tradeType eq 9}">selected="selected"</c:if>>退款</option>
                        </select>
                    </td>
                    <td>
            ...
            <c:forEach items="${ConsumeLogDTOList}" var="ral" varStatus="sort">
                <tr target="sid_user" rel="${ral.orderId }">
                    ...
                    <td>
                        ...
                        <c:if test="${ral.tradeType eq 6}">缴纳物业费</c:if>
                        <c:if test="${ral.tradeType eq 7}">缴纳停车费</c:if>
