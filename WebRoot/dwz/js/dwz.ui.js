...
function initUI(_box) {
    ...
    // 异步获取操作权限信息
    if (_box != null) {
        var _$panelBar = $("div.panelBar", $p);
        if (_$panelBar.find("ul.toolBar")) {
            // 获取澄放操作按钮的工具栏
            var _$toolBar = $("ul.toolBar", _$panelBar);
            if (_$toolBar != null) {
                // 获取form中的action地址，用于做查询条件
                var _$pageForm = $("form", $("div.pageHeader", $p)).attr("action");
                if (_$pageForm != null) {
                    var _start = _$pageForm.lastIndexOf("/") + 1;
                    var _end = _$pageForm.length;
                    // 从action中截取url
                    var _$url = _$pageForm.substring(_start, _end);
                    $.post("../admin/power_todo.do", {term: _$url}, function (data) {
                        _$toolBar.text("").append(data);
                        initPowerUI(_box);
                    }, "json");
                }
            }
    ...
    if ($.fn.combox) $("select.combox", $p)._comboxORef();

    if ($.fn.xheditor) {
        $("textarea.editor", $p).each(function () {
            var $this = $(this);
            // 设置默认的富文本域显示功能
            var tools = "Blocktag,Fontface,FontSize,Bold,Italic,Underline,Strikethrough,FontColor,BackColor,SelectAll,Removeformat";
            tools += ",|,Align,Outdent,Indent";
            tools += ",|,Link,Unlink,Img,Hr,Table";
            tools += ",|,Source,About";
            var op = {html5Upload: false, skin: 'nostyle', tools: $this.attr("tools") || tools};
            var upAttrs = [
...
function initPowerUI(_box) {
    ...
    $("a[target=dialog]", $p).each(function () {
        $(this).click(function (event) {
            ...
            // 如果是修改或者回复的操作，则先验证参数是否合法
            if (title.indexOf("修改") == 0 || title.indexOf("回复") == 0 || title.indexOf("编辑") == 0)
                $.pdialog._HOpen(url, rel, title, options);
            else
                $.pdialog.open(url, rel, title, options);
