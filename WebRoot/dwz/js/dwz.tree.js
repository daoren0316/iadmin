 ...
 (function($){
 	$.extend($.fn, {
		...
		_checkParent:function(){
            ...
            var topbox = $(">div>.ckbox", parent);
            topbox.removeClass("unchecked").removeClass("checked").removeClass(rClass).addClass(aClass);
            // HHang修改
            // 无论子集中的元素是否全部被选中，都提交父类元素的值(value)
            if(ckboxed>0)
                $(":checkbox", topbox).attr("checked","checked");
